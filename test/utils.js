...
const deployBank = async function(rootAddress,ownerAccount) {
    ...
    const simpleBank = await locklift.giver.deployContract({
        contract: SimpleBank,
        initParams: {
            nonce: locklift.utils.getRandomNonce()
        },
    ...
    simpleBank.afterRun = afterRun;

    simpleBank.setKeyPair(keyPair);

    const simpleBankBalance = await locklift.ton.getBalance(simpleBank.address);

    expect(simpleBankBalance.toNumber()).to.be.above(0, 'Bad bank balance');

    logger.log(`SimpleBank address: ${simpleBank.address}`);

    const {
        acc_type_name
    } = await locklift.ton.getAccountType(simpleBankBalance.address);
